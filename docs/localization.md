...
Pictures speaks the ten languages LineXinBar does: **German**, **English
...
is nothing to install, no translation package, no message directory and no
font to find.
...
```sh
LC_ALL=pl_PL.UTF-8 imagonsole
LC_ALL=en_US.UTF-8 imagonsole
```
...
A language written in a script the toolkit ships no face for needs one
bundled there first: `every_word_the_catalogs_carry_can_be_drawn` shapes every
line of these catalogs through the shipped faces alone and fails on the first
