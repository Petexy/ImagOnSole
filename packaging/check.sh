...
# `imagonsole --version` prints CARGO_PKG_VERSION, so the manifest is where a
...
[[ "$locked_version" == "$PACKAGE_VERSION" ]] \
    || package_die "Cargo.lock still says imagonsole $locked_version, not $PACKAGE_VERSION.
Run: scripts/bump-version.sh $PACKAGE_VERSION"
...
grep -Eq "^Version:[[:space:]]+${PACKAGE_VERSION//./\\.}\$" \
    "$PACKAGING_DIR/fedora/imagonsole.spec" \
    || package_die "fedora/imagonsole.spec does not declare version $PACKAGE_VERSION"
...
newest_release="$(awk '
    match($0, /<release version="[^"]+"/) {
        ...
        print line
        exit
    }' "$PROJECT_ROOT/data/io.github.petexy.imagonsole.metainfo.xml")"
[[ "$newest_release" == "$PACKAGE_VERSION" ]] \
    || package_die "the metainfo's newest release is $newest_release, not $PACKAGE_VERSION.
Add a <release> for $PACKAGE_VERSION to data/io.github.petexy.imagonsole.metainfo.xml"
...
grep -Eq "^BuildRequires:[[:space:]]+lxb-toolkit-devel >= ${toolkit_version//./\\.}\$" \
    "$PACKAGING_DIR/fedora/imagonsole.spec" \
    || package_die "fedora/imagonsole.spec does not build-require lxb-toolkit-devel >= $toolkit_version.
...
desktop_name="$(awk -F= '/^Name=/ { print substr($0, 6); exit }' \
    "$PROJECT_ROOT/data/imagonsole.desktop")"
...
program="$target_dir/release/imagonsole"
[[ -x "$program" ]] || package_die "missing release binary: $program
Run without --no-build, or build it first."
reported="$("$program" --version)"
[[ "$reported" == "imagonsole $PACKAGE_VERSION" ]] \
    || package_die "the binary reports '$reported', not 'imagonsole $PACKAGE_VERSION'"

package_note "staging the payload"
stage="$(package_work_dir imagonsole-check)"
cleanup() {
    if [[ -n "${stage:-}" && "$stage" == */imagonsole-check.* && -d "$stage" ]]; then
        rm -rf -- "$stage"
    fi
}
trap cleanup EXIT
...
for expected in \
    usr/bin/imagonsole \
    usr/share/applications/imagonsole.desktop \
    usr/share/icons/hicolor/scalable/apps/imagonsole.svg \
    usr/share/metainfo/io.github.petexy.imagonsole.metainfo.xml; do
...
exec_name="$(awk -F= '/^Exec=/ { print $2; exit }' \
    "$stage/usr/share/applications/imagonsole.desktop" | awk '{print $1}')"
icon_name="$(awk -F= '/^Icon=/ { print $2; exit }' \
    "$stage/usr/share/applications/imagonsole.desktop")"
...
if command -v desktop-file-validate >/dev/null 2>&1; then
    desktop-file-validate "$stage/usr/share/applications/imagonsole.desktop" \
...
if command -v appstreamcli >/dev/null 2>&1; then
    ...
    appstreamcli validate --no-net --pedantic \
        "$stage/usr/share/metainfo/io.github.petexy.imagonsole.metainfo.xml" \
...
package_note "all checks passed for imagonsole $PACKAGE_VERSION"
