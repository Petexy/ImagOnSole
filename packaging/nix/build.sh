...
Builds the flake's imagonsole package and writes a result symlink beneath the
artifact directory. The toolkit comes from the flake input rather than from
/usr, so this is the one target that does not need lxb-toolkit installed.
USAGE
}
