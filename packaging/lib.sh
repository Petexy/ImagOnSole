...
# manifest, the spec, the AppStream release list and `imagonsole --version`
# all have to agree with it. See scripts/bump-version.sh.
#
...
# at a fraction of RAM, and building there means building in memory. This is a
# GPU application: the locked graph is 400-odd crates, wgpu and naga among
# them, and a release build of it measures about 2.3 GiB on this machine before
# the dev-profile build that the test phase adds. There is no laptop where that
# fits in a tmpfs.
#
...
PACKAGE_WORK_ROOT="${IMAGONSOLE_WORK_DIR:-$PACKAGING_DIR/out/build}"
...
# about this project that surprises everybody who packages it. `lxb-render` is a
# Rust path dependency: cargo compiles those sources into this binary, so the
# finished program links no liblxb_*.so at all and a machine that runs it needs
# nothing of the toolkit installed. A machine that *builds* it needs the crate
# sources, which is what the toolkit's devel component puts under /usr/share.
#
# That path is what Cargo.toml names, so it is the same on every distribution
# here — and it is why this is checked rather than assumed. Cargo's own error
# for a missing path dependency names a directory and no package.
PACKAGE_TOOLKIT_CRATES="${LXB_TOOLKIT_CRATE_DIR:-/usr/share/lxb-toolkit/crates}"

require_toolkit_sources() {
    local wanted
    wanted="$(package_toolkit_requirement)"

    [[ -d "$PACKAGE_TOOLKIT_CRATES/lxb-render" ]] || package_die \
        "the toolkit's crate sources are not at $PACKAGE_TOOLKIT_CRATES/lxb-render.
...
# definition has to require. Read rather than repeated: a requirement written
# out four times is a requirement that is wrong in three places one day.
package_toolkit_requirement() {
    local version
    version="$(awk '
        /^lxb-render[[:space:]]*=/ {
    ...
    [[ -n "$version" ]] || package_die "could not read the lxb-render requirement from Cargo.toml"
    printf '%s\n' "$version"
}
...
# A relative IMAGONSOLE_WORK_DIR has to become absolute here rather than at
...
if [[ -n "${IMAGONSOLE_WORK_DIR:-}" ]]; then
    package_set_work_root "$IMAGONSOLE_WORK_DIR"
fi
...
Point the build somewhere with room using --work-dir DIR (or IMAGONSOLE_WORK_DIR),
