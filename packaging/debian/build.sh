...
RUSTFLAGS="${RUSTFLAGS:-} --remap-path-prefix=$PROJECT_ROOT=/usr/src/imagonsole-$PACKAGE_VERSION" \
    CARGO_TARGET_DIR="$target_dir" \
    cargo build --manifest-path "$PROJECT_ROOT/Cargo.toml" --release --locked

work="$(package_work_dir imagonsole-debian)"
cleanup() {
    if [[ -n "${work:-}" && "$work" == */imagonsole-debian.* && -d "$work" ]]; then
        rm -rf -- "$work"
    fi
}
trap cleanup EXIT
...
if command -v strip >/dev/null 2>&1; then
    strip --strip-unneeded "$package_root/usr/bin/imagonsole"
fi

shlib_depends=""
shlib_arguments=(-e"$package_root/usr/bin/imagonsole")
