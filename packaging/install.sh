...
Stages Imagonsole into DESTDIR. PREFIX defaults to /usr and --target-dir to
...
builder that has put it somewhere neither of those describes.
  ...
  bin/imagonsole
  share/applications/imagonsole.desktop
  share/icons/hicolor/scalable/apps/imagonsole.svg
  share/metainfo/io.github.petexy.imagonsole.metainfo.xml
...
binary="${binary:-${BINARY:-$target_dir/release/imagonsole}}"
[[ -x "$binary" ]] || package_die "missing release binary: $binary
Build it first: cargo build --release"

install -Dm0755 "$binary" "$install_root/bin/imagonsole"
install -Dm0644 "$PROJECT_ROOT/data/imagonsole.desktop" \
    "$install_root/share/applications/imagonsole.desktop"
...
install -Dm0644 "$PROJECT_ROOT/data/imagonsole.svg" \
    "$install_root/share/icons/hicolor/scalable/apps/imagonsole.svg"
# What GNOME Software, Discover and this store itself read to describe an
# application. Shipping it is how Imagonsole appears in the other two.
install -Dm0644 "$PROJECT_ROOT/data/io.github.petexy.imagonsole.metainfo.xml" \
    "$install_root/share/metainfo/io.github.petexy.imagonsole.metainfo.xml"

package_note "staged imagonsole $PACKAGE_VERSION into $install_root"
