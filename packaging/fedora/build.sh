...
work="$(package_work_dir imagonsole-fedora)"
cleanup() {
    if [[ -n "${work:-}" && "$work" == */imagonsole-fedora.* && -d "$work" ]]; then
        rm -rf -- "$work"
    fi
}
trap cleanup EXIT
...
archive_snapshot "$source_dir" "$work/SOURCES/imagonsole-$PACKAGE_VERSION.tar.gz"
install -m0644 "$script_dir/imagonsole.spec" "$work/SPECS/imagonsole.spec"
