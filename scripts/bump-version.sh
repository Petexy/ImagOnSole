...
# Cargo's manifest is one: `imagonsole --version` prints CARGO_PKG_VERSION, so
...
spec="$project_root/packaging/fedora/imagonsole.spec"
metainfo="$project_root/data/io.github.petexy.imagonsole.metainfo.xml"
...
packaging/fedora/imagonsole.spec and the AppStream release list. Every other
...
note "packaging/fedora/imagonsole.spec Version: -> $new_version"
awk -v version="$new_version" '
    /^Version:[[:space:]]/ && !done {
        printf "Version:        %s\n", version
        done = 1
...
if grep -Fq "<release version=\"$new_version\"" "$metainfo"; then
    ...
    note "data/io.github.petexy.imagonsole.metainfo.xml <- $new_version"
    today="$(date -u +%Y-%m-%d)"
...
echo "    packaging/fedora/imagonsole.spec   %changelog"
echo "    data/io.github.petexy.imagonsole.metainfo.xml   what this release changed"
